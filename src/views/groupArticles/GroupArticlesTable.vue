  ...
  <Card class="shadow-sm p-6 gap-4">
    ...
    <CardContent class="p-0 gap-0 overflow-x-auto mt-4">
      <Table>
        ...
        <TableBody>
          <TableRow v-for="(article, index) in groupArticles" :key="article.id" :class="{ 'bg-neutral-100/50': index % 2 !== 0 }"
          ...
          <TableRow v-if="groupArticles.length === 0">
            <TableCell colspan="6" class="text-center py-8 text-neutral-400">
              No group articles found matching search criteria.
            </TableCell>
          </TableRow>
        </TableBody>
      </Table>
    ...
    <form @submit.prevent="handleUpdateArticle">
      <Card class="shadow-2xl p-6 gap-6 w-[520px] relative">
        ...
        <CardHeader class="p-0 flex flex-col gap-1 pr-8">
          <h2 class="font-semibold text-lg leading-7">Update Group Article</h2>
        ...
        <CardContent class="flex p-0 flex-col gap-4 mt-6">
          <div class="flex flex-col gap-2">
            ...
            <Input id="update-name" placeholder="e.g. Introduction to Data Structures" v-model="updateForm.name" required />
          </div>
...
import type { IGroupArticle } from '@/interfaces/groupArticles';
...
const fetchArticlesData = () => {
  ...
  store.dispatch('groupArticles/findGroupArticlesAction', params);
};
...
const openUpdateModal = (article: IGroupArticle) => {
  updateForm.value = {
    id: article.id,
    name: article.name,
...
const handleUpdateArticle = async () => {
  try {
    await store.dispatch('groupArticles/updateGroupArticleAction', {
    ...
    fetchArticlesData();
    toast.success("Group article updated successfully.");
  } catch (error: any) {
    const errorMsg = error?.response?.data?.message || 'Failed to update group article.';
...
const handleDeleteArticle = async (id: number) => {
  try {
    await store.dispatch('groupArticles/deleteGroupArticleAction', id);
    fetchArticlesData();
    toast.success("Group article deleted successfully.");
  } catch (error: any) {
    const errorMsg = error?.response?.data?.message || 'Failed to delete group article.';
    toast.error(errorMsg);
  }
};
</script>
